...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:pass123@localhost:5432/mydatabase'
...
def search_professor():
    ...
    print(f"Received query in backend: '{query}'")  # Log the received query

    if not query:
        return jsonify([]), 200  # Return an empty list if no query

    # Filter your database or data based on the query parameter
    ...
    print(f"Matching professors: {professors}")  # Log matching professors
...
@app.route('/track_professor_view', methods=['POST'])
def track_professor_view():
    data = request.json
    ...
    if 'viewed_professors' not in session:
        session['viewed_professors'] = []

    # Append the professor to session history
    session['viewed_professors'].append({
        "id": professor.id,
        "name": professor.name,
        "department": professor.department
    })
    return jsonify({"message": "Professor view recorded"}), 200

@app.route('/viewing_history', methods=['GET'])
def viewing_history():
    if 'viewed_professors' in session:
        return jsonify(session['viewed_professors']), 200
    else:
